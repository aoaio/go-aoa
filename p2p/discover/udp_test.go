...
func newUDPTest(t *testing.T) *udpTest {
	...
	test.table, test.udp, _ = newUDP(test.localkey, test.pipe, realaddr, nil, "", nil, true)
	return test
}
...
func TestUDP_pingTimeout(t *testing.T) {
	...
	toid := NodeID{1, 2, 3, 4}
	if err := test.udp.ping(toid, toaddr, CommNet); err != errTimeout {
...
func TestUDP_findnodeTimeout(t *testing.T) {
	...
	result, err := test.udp.findnode(toid, toaddr, target, CommNet)
	if err != errTimeout {
		t.Error("expected timeout error, got", err)
	}
	if len(result) > 0 {
...
func TestUDP_findnode(t *testing.T) {
	...
	test.table.stuff(nodes.entries, CommNet)
	...
	test.packetIn(nil, findnodePacket, &findnode{Target: testTarget, Expiration: futureExp, NetType: CommNet})
	expected := test.table.closest(targetHash, bucketSize, CommNet)
...
func TestUDP_findnodeMultiReply(t *testing.T) {
	...
	go func() {
		...
		ns, err := test.udp.findnode(rid, test.remoteaddr, testTarget, CommNet)
		if err != nil && len(ns) == 0 {
			errc <- err
		} else {
			resultc <- ns
