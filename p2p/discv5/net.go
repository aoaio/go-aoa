...
loop:
	for {
		...
		case pkt := <-net.read:
			...
			log.Debug(fmt.Sprintf("<<< (%d) %v from %x@%v: %v -> %v (%v)",
		...
		case timeout := <-net.timeout:
			...
			log.Debug(fmt.Sprintf("--- (%d) %v for %x@%v: %v -> %v (%v)",
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		log.Info(fmt.Sprintf("seed node (age %s): %v", age, n))
...
const (
	...
	// Packet type events.
	// These correspond to packet walletType in the UDP protocol.
	pingPacket
	pongPacket
	findnodePacket
	neighborsPacket
	...
	// the packet type range (packet walletType are encoded as a single byte).
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	log.Debugf("Pinging remote node %v", n.ID)
...
func (net *Network) handlePing(n *Node, pkt *ingressPacket) {
	log.Debugf("Handling remote ping %v", n.ID)
	ping := pkt.data.(*ping)
	n.TCP = ping.From.TCP
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	log.Debugf("Handling known pong %v", n.ID)
	...
	if err == nil {
		...
		log.Debugf("Failed to convert pong to ticket, err=%v", err)
	}
	n.pingEcho = nil
	n.pingTopics = nil
	return err
