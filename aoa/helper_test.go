...
func newTestProtocolManager(mode downloader.SyncMode, blocks int, generator func(int, *core.BlockGen), newtx chan<- []*types.Transaction) (*ProtocolManager, error) {
	var (
		...
		blockchain, _ = core.NewBlockChain(db, gspec.Config, dpos.New(), vm.Config{}, nil)
		engine        = dpos.New()
	)
	...
	pm, err := NewProtocolManager(gspec.Config, mode, DefaultConfig.NetworkId, &testTxPool{added: newtx}, engine, blockchain, db, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	pm.Start(1000)
...
func (p *testTxPool) Pending() (map[common.Address]types.Transactions, error) {
	...
	for _, tx := range p.pool {
		from, _ := types.Sender(types.NewAuroraSigner(new(big.Int)), tx)
...
func (p *testTxPool) PoolSigner() types.Signer {
	return p.PoolSigner()
}
...
func newTestTransaction(from *ecdsa.PrivateKey, nonce uint64, datasize int) *types.Transaction {
	tx := types.NewTransaction(nonce, common.Address{}, big.NewInt(0), 100000, big.NewInt(0), make([]byte, datasize), 0, nil, nil, nil, nil, "")
	tx, _ = types.SignTx(tx, types.NewAuroraSigner(new(big.Int)), from)
	return tx
}
