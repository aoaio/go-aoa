...
func (pm *ProtocolManager) txsyncLoop() {
	...
	for {
		...
		case s := <-pm.txsyncCh:
			...
			if !sending {
				// Add log
				log.Info("begin sync trx", "trx length：", len(pack.txs))
				send(s)
			}
		case err := <-done:
			sending = false
...
func (pm *ProtocolManager) synchroniseWithHigherPeers(peer *peer, block *types.Block) {
	if peer == nil {
		return
	}
	currentBlock := pm.blockchain.CurrentBlock()
	td := pm.blockchain.GetTd(currentBlock.ParentHash(), currentBlock.NumberU64()-1)
	if td == nil {
		return
	}
