...
func (db *ephemeralDatabase) Prune(root common.Hash) {
	...
	trieSync := state.NewStateSync(root, db.diskdb)
...
func (api *PrivateDebugAPI) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
	...
	go func() {
		...
		for number = start.NumberU64() + 1; number <= end.NumberU64(); number++ {
			...
			delegateDB, err := delegatestate.New(start.DelegateRoot(), delegatestate.NewDatabase(db))
			if err != nil {
				failed = err
				break
			}
			...
			// delegate db
			delegateroot, err := delegateDB.CommitTo(db, false)
			...
			if err := delegateDB.Reset(delegateroot); err != nil {
				failed = err
				break
			}
...
func (api *PrivateDebugAPI) traceTx(ctx context.Context, message core.Message, vmctx vm.Context, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	case *vm.StructLogger:
		return &aoaapi.ExecutionResult{
			Gas:         gas,
			Failed:      failed,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  aoaapi.FormatLogs(tracer.StructLogs()),
		}, nil
