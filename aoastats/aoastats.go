...
// Package aoastats implements the network stats reporting service.
package aoastats
...
// Service implements an Aurora netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server      // Peer-to-peer server to retrieve networking infos
	aoa    *aoa.Aurora      // Full Aurora service if monitoring a full node
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // aoastats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the aoastats server
	start := time.Now()
