...
func opCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	ret, returnGas, err := evm.Call(contract, toAddr, args, gas, 0, value)
	if err != nil {
		stack.push(evm.interpreter.intPool.getZero())
	} else {
...
func opBalanceOf(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	asset, addr := common.BigToAddress(stack.pop()), common.BigToAddress(stack.pop())
	balance := evm.StateDB.GetAssetBalance(addr, asset)
	stack.push(new(big.Int).Set(balance))
	return nil, nil
}

func opTransferAsset(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	value, aid, to := stack.pop(), stack.pop(), stack.pop()
	value = math.U256(value)
	asset, toaddr := common.BigToAddress(aid), common.BigToAddress(to)
