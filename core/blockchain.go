...
type BlockChain struct {
	...
	innerTxDb     watch.InnerTxDb //database to store inner transactions
...
// rollback with header block number
...
// delegate state returns a new mutable db based on the current HEAD block.
...
func (bc *BlockChain) preInsertChain(block *types.Block) error {
	...
	if err != nil {
		...
		if err == consensus.ErrFutureBlock {
			...
			// is exceeded the block is discarded and processed at a later time
			// if given.
...
// SubscribeLogsEvent registers a subscription of []*walletType.Log.
