...
//type gasHeap []*walletType.Transaction
//
//func (h gasHeap) Len() int           { return len(h) }
//func (h gasHeap) Less(i, j int) bool { return h[i].GasPrice().Cmp(h[j].GasPrice()) > 0 }
//func (h gasHeap) Swap(i, j int)      { h[i], h[j] = h[j], h[i] }
//
//func (h *gasHeap) Push(x interface{}) {
//	*h = append(*h, x.(*walletType.Transaction))
//}
//
...
//}
//
//func (h *gasHeap) Get(hash common.Hash) (int, *walletType.Transaction) {
//	for index, tx := range *h {
//		if hash == tx.Hash() {
//			return index, tx
//		}
...
//}
//
//func (t *gasHeap) Remove(index int) common.Hash {
//	removeTx := (*t)[index]
...
type txTypeList struct {
	all   *map[common.Hash]*types.Transaction // Pointer to the map of all transactions
	items *txPoolSorter                       // Heap of prices of all the stored transactions
	//stales int                                 // Number of stale price points to (re-heap trigger)
}
